        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (response.error) {
                    ...
                    price.value = formatPrice(info.totalPrice);
                    tax.value = formatPrice(info.taxPrice);
                    ipaca.value = formatPrice(info.ipacaPrice);
                    finprice.value = formatPrice(info.finalPrice);
                }
            } else {
...
function formatPrice(price) {
    let formatted = price.toLocaleString(undefined, { 
        useGrouping: true, 
